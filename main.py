...
async def process_receipt(user_id: str = Form(...), description: str = Form(...), country: str = Form(...)):
    ...
    # --- DYNAMIC SIZING & ANTI-CATCHER LOGIC ---
    desc_lower = description.lower()
    is_pos = any(word in desc_lower for word in ["pos", "store", "market", "shop", "thermal", "slip"])

    if is_pos:
        # 80mm width is standard for POS/Thermal receipts
        pdf = FPDF(format=(80, 200)) 
        pdf.add_page()
        pdf.set_font("Courier", 'B', 14) # Courier looks like real thermal print
        ...
        # A4 LUXURY BANK STYLE
        pdf = FPDF(format='A4')
        pdf.add_page()
        
        # Professional Header Bar
        pdf.set_fill_color(30, 40, 60) # Navy
