...
# --- THE FIXED & STRICT VERIFICATION ---
@app.post("/verify_payment")
async def verify(user_id: str = Form(...), plan: str = Form(...), file: UploadFile = File(...)):
    db = load_db()
    
    # Process image to make text pop out
    img = Image.open(file.file).convert('L') # Greyscale
    img = ImageOps.autocontrast(img) # Make text sharper
    text = pytesseract.image_to_string(img)
    
    # LOGIC: Must find your specific info
    ...
    # ALL must match to prevent cheating
    ...
    return {"status": "error", "message": "Verification failed! Screenshot invalid."}
